...
## Architecture: Terminal Claude Access
...
- **Backend (Express - `server.js`)**: A local proxy server that handles:
    ...
    - **Anthropic Integration**: Uses the `@anthropic-ai/sdk` to securely call Claude using the `ANTHROPIC_API_KEY` from the local environment.
...
2. `src/utils/claude.js` sends a POST request to `http://localhost:3001/api/scrape`.
...
5. Backend calls Claude and returns the structured JSON FAQs.
